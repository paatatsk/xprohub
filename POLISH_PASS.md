...
1. **Task library audit (188 → ~80)** — narrow-and-deep for cold-start density in the Park Slope launch. A 1–2 hour conversation (Paata + chat-Claude) + bulk `UPDATE task_library SET is_active = false`. See "Task library audit" entry below.
2. **hello@xprohub.com email routing** — the live legal pages (privacy/terms) and app reference this address; set up Cloudflare Email Routing → Gmail so it isn't a dead contact address. See "Cloudflare Email Routing" entry.
3. **(minor) job-bids stale bid list after hire** — cosmetic: accepted bid still shows Accept/Decline if user navigates back. Fix = refetch on focus. Not blocking, but feels buggy. See entry below.

**Launch-day manual steps** (not code — do these at submission time):
- Swap Stripe test key → live key: `pk_test_` → `pk_live_` in `app/_layout.tsx` (publishable key is safe to commit; this is just the test→live flip).
- Confirm `hello@xprohub.com` routing is live before it appears in App Store metadata.

DONE (shipped, struck from active list): proactive content moderation (2026-06-18), stub-screen cleanup (all six confirmed removed).
...
Note: task density + the parked safety/education layer form a cluster — "honest task density + light safety guidance" — arguably the most important pre-launch theme for an in-person marketplace, more than any visual polish.
