...
const MANDATORY: { id: ItemId; emoji: string; label: string; desc: string }[] = [
  { id: 'email', emoji: '📧', label: 'Business Email', desc: 'Verify your company email address' },
  { id: 'phone', emoji: '📱', label: 'Phone Number', desc: 'Business contact number' },
  { id: 'reg_docs', emoji: '📄', label: 'Registration Docs', desc: 'Company registration certificate' },
  { id: 'tax_id', emoji: '🔢', label: 'Tax ID Number', desc: 'Your business tax identification number' },
];

const OPTIONAL: { id: ItemId; emoji: string; label: string; desc: string; reward: string }[] = [
  { id: 'website', emoji: '🌐', label: 'Company Website', desc: 'Your official business website', reward: '+10 Trust Points' },
  { id: 'linkedin', emoji: '💼', label: 'LinkedIn Page', desc: 'Your company LinkedIn profile', reward: '+15 Trust Points' },
  { id: 'bank', emoji: '🏦', label: 'Bank Account', desc: 'For fast payments to workers', reward: '+20 Trust Points' },
  { id: 'references', emoji: '⭐', label: 'References', desc: 'Past worker or client references', reward: '+25 Trust Points' },
];

const getTrustLevel = (completed: Set<ItemId>) => {
  const mandatoryDone = ['email', 'phone', 'reg_docs', 'tax_id'].every(id =>
    completed.has(id as ItemId)
  );
  const optionalCount = ['website', 'linkedin', 'bank', 'references'].filter(id =>
    completed.has(id as ItemId)
  ).length;
  ...
  if (optionalCount === 0) {
    return {
      ...
      color: '#4A9EDB',
      emoji: '🔵',
      message: 'Add optional items to reach Gold Business status!',
    };
  }
  if (optionalCount >= 3) {
    return {
      ...
      color: '#C9A84C',
      emoji: '🏆',
      message: 'Maximum trust — workers will love working with you!',
    };
  }
  return {
    level: 2,
    ...
    color: '#4CAF7A',
    emoji: '🟢',
    message: `${3 - optionalCount} more item${3 - optionalCount > 1 ? 's' : ''} to reach Gold Business!`,
  };
};

export default function BusinessTrustSetupScreen() {
  ...
  const remainingCount = ['email', 'phone', 'reg_docs', 'tax_id'].filter(
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Step 2 of 3 — Verify your business</Text>
        ...
        <Text style={styles.sectionSub}>More you add, more workers trust you</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          onPress={() => router.push('/business-profile-builder')}
